...
class PS_Delete {
	...
	{		
		if ( ! $this->CI->Project_type->delete( $project_id )) {
	...
	// }
	 ...
	 function delete_postedby( $id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Postedby->delete( $id )) {
		// if there is an error in deleting category,
		...
		if ( $enable_trigger ) {
		// if execute_trigger is enable, trigger to delete wallpaper related data
			if ( ! $this->delete_postedby_trigger( $id )) {
	...
	*/
	...
	function delete_item_amenity( $item_id )
	{		
		$conds = array( 'item_id' => $item_id );
		if ( ! $this->CI->Item_amenity->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
	}	

	function delete_color( $post_id )
	{		
		$conds = array( 'post_id' => $post_id );
	...
	{		
		$conds = array( 'id' => $id );
		if ( ! $this->CI->PropertyBy->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
		return true;
	...
	{		
		$conds = array( 'id' => $id );
		if ( ! $this->CI->Amenity->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
		return true;
	...
	{		
		$conds = array( 'id' => $id );
		if ( ! $this->CI->Agent->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
		return true;
	...
	{		
		$conds = array( 'id' => $id );
		if ( ! $this->CI->Pricetype->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
		return true;
