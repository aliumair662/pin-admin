	...
	function index() {
        ...
        if ( $user_id !='' ) {
           ...
           if (!empty($user)) {
               ...
               $user_data['swipe_profile_hidden'] = ( $user->swipe_profile_hidden == 1 ) ? 0 : 1;
		       
               if ( ! $this->User->save( $user_data, $user_id )) {
		
					$this->set_flash_msg( 'error', 'Something wrong.');
				} else {

					$this->set_flash_msg( 'success', get_msg( 'success_user_edit' ));
				}

               redirect( $this->module_site_url()."/index/".$this->uri->segment( 4 ));
           }
        }
